...
abstract class Single_Base extends Archive_Single_Base
{
    ...
    {
        return thepack_builder()->elementor()->widgets_manager->get_widget_types('lakit-post-content');
    }
    ...
    public function get_elements_data($status = Document::STATUS_PUBLISH)
    {
        ...
        if (thepack_builder()->elementor()->preview->is_preview_mode() && self::get_property('location') === Module::instance()->get_locations_manager()->get_current_location()) {
            $has_the_content = false;

            $depended_widget = $this->get_depended_widget();

            thepack_builder()->elementor()->db->iterate_data($data, function ($element) use (&$has_the_content, $depended_widget) {
