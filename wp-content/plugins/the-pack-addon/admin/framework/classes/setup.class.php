...
if (!class_exists('CSF')) {
    class CSF
    {
        ...
        public static function includes()
        {
            ...
            $fields = apply_filters('csf_fields', [
                'accordion',
                'background',
                'backup',
                'border',
        ...
        public static function add_admin_enqueue_scripts()
        {
            ...
            if (apply_filters('csf_fa4', false)) {
                ...
                wp_enqueue_style('csf-fa5', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.3/css/all' . $min . '.css', [], '5.15.3', 'all');
                wp_enqueue_style('csf-fa5-v4-shims', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.3/css/v4-shims' . $min . '.css', [], '5.15.3', 'all');
            }

            // Main style
            wp_enqueue_style('csf', self::include_plugin_url('assets/css/style' . $min . '.css'), [], self::$version, 'all');

            // Main RTL styles
            if (is_rtl()) {
                wp_enqueue_style('csf-rtl', self::include_plugin_url('assets/css/style-rtl' . $min . '.css'), [], self::$version, 'all');
            }

            // Main scripts
            wp_enqueue_script('csf-plugins', self::include_plugin_url('assets/js/plugins' . $min . '.js'), [], self::$version, true);
            wp_enqueue_script('csf', self::include_plugin_url('assets/js/main' . $min . '.js'), ['csf-plugins'], self::$version, true);
        ...
        public static function add_typography_enqueue_styles()
        {
            if (!empty(self::$webfonts)) {
                if (!empty(self::$webfonts['enqueue'])) {
                    ...
                    wp_enqueue_style('csf-google-web-fonts', esc_url(add_query_arg($query, '//fonts.googleapis.com/css')), [], null);
                }
