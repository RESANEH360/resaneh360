...
class Locations_Manager
{
    ...
    public function enqueue_styles()
    {
        ...
        if (!empty($css_files)) {
            thepack_builder()->elementor()->frontend->enqueue_styles();
    ...
    public function template_include($template)
    {
        ...
        if (is_singular()) {
            $document = thepack_builder()->elementor()->documents->get_doc_for_frontend(get_the_ID());
         ...
         */
        $page_templates_module = thepack_builder()->elementor()->modules_manager->get_modules('page-templates');
    ...
    public function do_location($location)
    {
        ...
        if (is_singular()) {
            thepack_builder_helper()->set_global_authordata();
        }
    ...
    {
        $inspector_enabled = method_exists(thepack_builder()->elementor()->inspector, 'is_enabled') && thepack_builder()->elementor()->inspector->is_enabled();
        ...
        if (isset($args['template'])) {
            $title[] = thepack_builder()->elementor()->inspector->parse_template_path($args['template']);
        }

        $title = implode(' > ', $title);

        thepack_builder()->elementor()->inspector->add_log('Theme', $title, $url);
    }
