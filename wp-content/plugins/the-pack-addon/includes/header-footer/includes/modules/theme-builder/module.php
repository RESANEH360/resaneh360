<?php
namespace ThePackThemeBuilder\Modules\ThemeBuilder;

use Elementor\Core\Admin\Admin_Notices;
use Elementor\Core\App\App;
use Elementor\Core\Base\Document;
use Elementor\TemplateLibrary\Source_Local;
use ThePackThemeBuilder\Modules\ThemeBuilder\Classes;
use ThePackThemeBuilder\Modules\ThemeBuilder\Documents\Single;
use ThePackThemeBuilder\Modules\ThemeBuilder\Documents\Theme_Document;
...
class Module extends \Elementor\Core\Base\Module
{
    ...
    {
        return thepack_builder()->elementor()->preview->is_preview_mode() || is_preview();
    }

    public static function get_public_post_types($args = [])
    {
        $post_types = thepack_builder_helper()->get_post_types();
    ...
    public function get_document($post_id)
    {
        ...
        try {
            $document = thepack_builder()->elementor()->documents->get($post_id);
        } catch (\Exception $e) {
            // Do nothing.
            unset($e);
        }
    ...
    public function localize_settings($settings)
    {
        $settings = array_replace_recursive($settings, [
            ...
            'urls' => [
                'modules' => thepack_builder()->plugin_url('')
            ]
        ]);
    ...
    {
        $controls_manager = thepack_builder()->elementor()->controls_manager;
    ...
    public function create_new_dialog_types($types)
    {
        ...
        foreach ($types as $type => $label) {
            $document_type = thepack_builder()->elementor()->documents->get_document_type($type);
			...
			<div class="elementor-form-field__select__wrapper">
				<select id="elementor-new-template__form__post-type" class="elementor-form-field__select" name="<?php echo Single::REMOTE_CATEGORY_META_KEY; ?>">
                    ...
                    foreach ($post_types as $post_type => $label) {
                        $doc_type = thepack_builder()->elementor()->documents->get_document_type($post_type);
    ...
    {
        thepack_builder()->elementor()->common->add_template(__DIR__ . '/views/panel-template.php');
    }
    ...
    {
        add_submenu_page('the-pack', '', __('Theme Builder', 'thepack'), 'publish_posts', $this->get_admin_templates_url(true));
    }

    public function print_new_theme_builder_promotion($views)
    {
        /** @var Source_Local $source */
        $source = thepack_builder()->elementor()->templates_manager->get_source('local');
        ...
        if (self::ADMIN_LIBRARY_TAB_GROUP === $current_tab_group) {
             ...
             */
            $admin_notices = thepack_builder()->elementor()->admin->get_component('admin-notices');
            ...
            $admin_notices->print_admin_notice([
                ...
                'button' => [
                    ...
                    'url' => thepack_builder()->elementor()->app->get_settings('menu_url'),
                ],
            ]);
        }
    ...
    public function enqueue_editor_scripts()
    {
        wp_enqueue_style(
            ...
            thepack_builder()->plugin_url('assets/css/thepack-kit-editor-conditions.css'),
            [],
            thepack_builder()->get_version()
        );

        wp_enqueue_script(
            'thepack-kit-editor-conditions',
            thepack_builder()->plugin_url('assets/js/thepack-kit-editor-conditions.js'),
            [
                'backbone-marionette',
                'elementor-common',
